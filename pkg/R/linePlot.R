linePlot <- function(){
    ...
    ############################################################
    titlesFrame <- tkframe(top)
    titlesVariable <- tclVar(gettextRcmdr(""))
    titlesField <- tkentry(titlesFrame, width="40", textvariable=titlesVariable)
    ############################################################
    onOK <- function(){
        y <- getSelection(yBox)
        x <- getSelection(xBox)
        ...
        ############################################################
        titles <- tclvalue(titlesVariable)
        ############################################################
        ...
        command <- paste("matplot(", .activeDataSet, "$", x, ", ", .activeDataSet, "[, ",
            paste("c(", paste(paste('"', y, '"', sep=""), collapse=","), ")", sep=""),
            '], type="b", lty=1, main="', titles, '", ylab="', axisLabel, '"', pch, ")", sep="")
        ####################################################################
        logger(command)
        justDoIt(command)
        if (legend && length(y) > 1){
    ...
    ###################################################################
    tkgrid(tklabel(titlesFrame, text=gettextRcmdr("Title: ")), titlesField, sticky="w")
    tkgrid(titlesFrame, sticky="w")
    ###################################################################
