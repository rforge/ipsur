...
powerTtest <- function() {
    ...
    ##############
    # Buttons
    radioButtons(optionsFrame, "type", buttons=c("two.sample", "one.sample", "paired"),
        labels=gettextRcmdr(c("Two sample", "One sample", "Paired")), title=gettextRcmdr("Type of t Test"))
    radioButtons(optionsFrame, "alternative", buttons=c("two.sided", "one.sided"),
        labels=gettextRcmdr(c("Two sided", "One sided")), title=gettextRcmdr("Alternative Hypothesis"))
    ...
    onOK <- function(){
        ...
        if ( ssizeValue=="" ){
             ...
             errorCondition(recall=powerTtest,
                message=gettextRcmdr("Exactly one field must be left empty."))
                return()
        }

        # Construct the R command
        command <- paste( 'power.t.test(n=',
                          ssizeValue,
                          ', delta=',
                          deltaValue,
                          ', sd=',
    ...
    ####################################
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters: (leave exactly one field blank)"), fg="blue"), columnspan=2, sticky="w")
    ################
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Number of observations (per group):")), entrySsize, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("True difference in means:")), entryDelta, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Standard deviation:")), entrySd, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Significance level (Type I error probability):")), entrySig.level, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Power of test (1 minus Type II error probability):")), entryPower, sticky="w")
    ################
    tkgrid(typeFrame, sticky="w", columnspan=2)
    tkgrid(optionsFrame, sticky="w")
    tkgrid(buttonsFrame, sticky="w")
    tkgrid(alternativeFrame, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Use strict interpretation in two-sided case:")), strictCheckBox, sticky="w")
...
powerProptest <- function() {
    ...
    onOK <- function(){
        ...
        if ( ssizeValue=="" ){
             ...
             errorCondition(recall=powerProptest,
                message=gettextRcmdr("Exactly one field must be left empty."))
                return()
        }

        # Construct the R command
        command <- paste( 'power.prop.test(n=',
                          ssizeValue,
                          ', p1=',
                          p1Value,
                          ', p2=',
    ...
    ################
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Number of observations (per group):")), entrySsize, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("probability in one group:")), entryP1, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("probability in other group:")), entryP2, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Significance level (Type I error probability):")), entrySig.level, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Power of test (1 minus Type II error probability):")), entryPower, sticky="w")
    ################
    tkgrid(optionsFrame, sticky="w")
...
powerAnovatest <- function() {
    ...
    onOK <- function(){
        ...
        if ( ssizeValue=="" ){
             ...
             errorCondition(recall=powerAnovatest,
                message=gettextRcmdr("Exactly one field must be left empty."))
                return()
        }

        # Construct the R command
        command <- paste( 'power.anova.test(groups=',
                          groupsValue,
                          ', n=',
                          ssizeValue,
                          ', between.var=',
    ...
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters: (leave exactly one field blank)"), fg="blue"), columnspan=2, sticky="w")
    ################
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Number of groups:")), entryGroups, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Number of observations (per group):")), entrySsize, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Between group variance:")), entryBetween.var, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Within group variance:")), entryWithin.var, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Significance level (Type I error probability):")), entrySig.level, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Power of test (1 minus Type II error probability):")), entryPower, sticky="w")
    ################
    tkgrid(optionsFrame, sticky="w")
