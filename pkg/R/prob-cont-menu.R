...
betaQuantiles.ipsur <- function(){
    ...
    onOK <- function(){
        ...
        doItAndPrint(paste("qbeta(c(", quantiles, "), shape1=", shape1,
            ", shape2=", shape2, ", ncp=", ncp,", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }

    OKCancelHelp(helpSubject="qbeta")
    tkgrid(tklabel(top, text=gettextRcmdr("Probabilities")), quantilesEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("shape1 ")), shape1Entry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("shape2 ")), shape2Entry, sticky="e")
...
betaProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        if ("" == quantiles) {
              errorCondition(recall=betaProbabilities.ipsur, message=gettextRcmdr("No probabilities specified."))
              return()
        }
        shape1 <- tclvalue(shape1Var)
        shape2 <- tclvalue(shape2Var)
        ...
        # Do some error checking
        if ( shape1==""){
              errorCondition(recall=betaProbabilities.ipsur, message=gettextRcmdr("Shape1 parameter not specified."))
              return()
        }
        if ( shape2==""){
                errorCondition(recall=betaProbabilities.ipsur, message=gettextRcmdr("Shape2 parameter not specified"))
                return()
        }
        if ( ncp=="" ){
                errorCondition(recall=betaProbabilities.ipsur, message=gettextRcmdr("Noncentrality parameter not specified."))
                return()
        }
        tail <- tclvalue(tailVar)
        doItAndPrint(paste("pbeta(c(", quantiles, "), shape1=", shape1,
            ", shape2=", shape2, ", ncp=", ncp,", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
...
betaDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        if ( shape1=="" ){
                errorCondition(recall=betaDistributionPlot.ipsur, message=gettextRcmdr("Shape1 parameter not specified."))
                return()
        }
        if ( shape2=="" ){
                errorCondition(recall=betaDistributionPlot.ipsur, message=gettextRcmdr("Shape2 parameter not specified."))
                return()
        }
        if ( ncp=="" ){
                errorCondition(recall=betaDistributionPlot.ipsur, message=gettextRcmdr("Noncentrality parameter not specified."))
                return()
        }
...
cauchyQuantiles.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        doItAndPrint(paste("qcauchy(c(", quantiles, "), location=", location,
            ", scale=", scale1, ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
...
cauchyProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        if ("" == probabilities) {
              errorCondition(recall=cauchyProbabilities.ipsur, message=gettextRcmdr("No values specified."))
              return()
        }
        location <- tclvalue(locationVar)
        scale1 <- tclvalue(scale1Var)
        tail <- tclvalue(tailVar)
        if ( location =="" ){
              errorCondition(recall=cauchyProbabilities.ipsur, message=gettextRcmdr("The location parameter was not specified."))
              return()
        }
        if ( scale1 ==""  ){
              errorCondition(recall=cauchyProbabilities.ipsur, message=gettextRcmdr("The scale parameter was not specified."))
              return()
        }
...
cauchyDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        fun <- tclvalue(functionVar)
        if ( location =="" ){
              errorCondition(recall=cauchyDistributionPlot.ipsur, message=gettextRcmdr("The location parameter was not specified."))
              return()
        }
        if ( scale1 =="" ){
...
chisqQuantiles.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        tail <- tclvalue(tailVar)
        doItAndPrint(paste("qchisq(c(", quantiles, "), df=", df, ", ncp=", ncp,
            ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
    OKCancelHelp(helpSubject="qchisq")
    tkgrid(tklabel(top, text=gettextRcmdr("Probabilities")), quantilesEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("df (degrees of freedom)")), dfEntry, sticky="e")
...
chisqProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        probabilities <- gsub(" ", ",", tclvalue(probabilitiesVar))
        if ("" == probabilities) {
            errorCondition(recall=chisqProbabilities.ipsur, message=gettextRcmdr("No values specified."))
            return()
        }
        df <- tclvalue(dfVar)
        if ( df =="" ) {
            errorCondition(recall=chisqProbabilities.ipsur, message=gettextRcmdr("Degrees of freedom not specified."))
            return()
        }
        ncp <- tclvalue(ncpVar)
        if ( ncp ==""  ){
            errorCondition(recall=chisqProbabilities.ipsur, message=gettextRcmdr("The noncentrality parameter was not specified."))
            return()
        }
        tail <- tclvalue(tailVar)
        doItAndPrint(paste("pchisq(c(", probabilities, "), df=", df, ", ncp=", ncp,
            ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
...
chisquareDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        df <- tclvalue(dfVar)
        if ( df ==""  ){
              errorCondition(recall=chisquareDistributionPlot.ipsur, message=gettextRcmdr("The degrees of freedom were not specified."))
              return()
        }
        ncp <- tclvalue(ncpVar)
        if ( ncp ==""  ){
              errorCondition(recall=chisquareDistributionPlot.ipsur, message=gettextRcmdr("The noncentrality parameter was not specified."))
              return()
        }
        fun <- tclvalue(functionVar)
		if(fun=="Density"){
...
expProbabilities.ipsur  <- function(){
...
expDistributionPlot.ipsur  <- function(){
...
FQuantiles.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        doItAndPrint(paste("qf(c(", quantiles, "), df1=", df1,
            ", df2=", df2, ", ncp=", ncp,", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
    OKCancelHelp(helpSubject="qf")
    tkgrid(tklabel(top, text=gettextRcmdr("Probabilities")), quantilesEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("df1 (numerator degrees of freedom)")), df1Entry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("df2 (denominator degrees of freedom)")), df2Entry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("ncp (noncentrality parameter)")), ncpEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("Lower tail")), lowerTailButton, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("Upper tail")), upperTailButton, sticky="e")
    tkgrid(buttonsFrame, columnspan=2, sticky="w")
...
FDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        df2 <- tclvalue(df2Var)
        if ( df1=="" || df2=="" )  {
              errorCondition(recall=FDistributionPlot.ipsur, message=gettextRcmdr("Degrees of freedom not specified."))
              return()
        }
        ncp <- tclvalue(ncpVar)
        if ( ncp=="" ){
              errorCondition(recall=FDistributionPlot.ipsur, message=gettextRcmdr("The noncentrality parameter was not specified."))
              return()
        }
...
gammaQuantiles.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        doItAndPrint(paste("qgamma(c(", quantiles, "), shape=", shape,
            ", rate=", scale1, ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
    OKCancelHelp(helpSubject="qgamma")
    tkgrid(tklabel(top, text=gettextRcmdr("Probabilities")), quantilesEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("shape ")), shapeEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("rate (=1/scale)")), scale1Entry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("Lower tail")), lowerTailButton, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("Upper tail")), upperTailButton, sticky="e")
    tkgrid(buttonsFrame, sticky="w", columnspan=2)
    tkgrid.configure(quantilesEntry, sticky="w")
...
gammaProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        probabilities <- gsub(" ", ",", tclvalue(probabilitiesVar))
        if ("" == probabilities) {
              errorCondition(recall=gammaProbabilities.ipsur, message=gettextRcmdr("No values specified."))
              return()
        }
        shape <- tclvalue(shapeVar)
        scale1 <- tclvalue(scale1Var)
        tail <- tclvalue(tailVar)
        if ( shape ==""  ){
              errorCondition(recall=gammaProbabilities.ipsur, message=gettextRcmdr("The shape parameter was not specified."))
              return()
        }
        if ( scale1 ==""  ){
              errorCondition(recall=gammaProbabilities.ipsur, message=gettextRcmdr("The rate parameter was not specified."))
              return()
        }
...
gammaDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        fun <- tclvalue(functionVar)
        if ( shape ==""  ){
              errorCondition(recall=gammaDistributionPlot.ipsur, message=gettextRcmdr("The shape parameter was not specified."))
              return()
        }
        if ( scale1 ==""  ){
...
lognormalQuantiles.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        quantiles <- gsub(" ", ",", tclvalue(quantilesVar))
        if ("" == quantiles) {
              errorCondition(recall=lognormalQuantiles.ipsur, message=gettextRcmdr("No probabilities specified."))
              return()
        }
        ...
        # Do some error checking
        if ( logmu =="" ){
              errorCondition(recall=lognormalQuantiles.ipsur, message=gettextRcmdr("The mean was not specified."))
              return()
        }
        if ( logsigma ==""  ){
              errorCondition(recall=lognormalQuantiles.ipsur, message=gettextRcmdr("The standard deviation was not specified."))
              return()
        }
        doItAndPrint(paste("qlnorm(c(", quantiles, "), meanlog=", logmu,
            ", sdlog=", logsigma, ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }

    OKCancelHelp(helpSubject="qlnorm")
    tkgrid(tklabel(top, text=gettextRcmdr("Probabilities")), quantilesEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("meanlog (mean of dist'n on log scale)")), logmuEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("sdlog (std dev of dist'n on log scale)")), logsigmaEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("Lower tail")), lowerTailButton, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("Upper tail")), upperTailButton, sticky="e")
    tkgrid(buttonsFrame, sticky="w", columnspan=2)
    tkgrid.configure(quantilesEntry, sticky="w")
...
lognormalProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        probabilities <- gsub(" ", ",", tclvalue(probabilitiesVar))
        if ("" == probabilities) {
              errorCondition(recall=lognormalProbabilities.ipsur, message=gettextRcmdr("No values specified."))
              return()
        }
        logmu <- tclvalue(logmuVar)
        logsigma <- tclvalue(logsigmaVar)
        tail <- tclvalue(tailVar)
        if ( logmu =="" ){
              errorCondition(recall=lognormalProbabilities.ipsur, message=gettextRcmdr("The mean was not specified."))
              return()
        }
        if ( logsigma ==""  ){
              errorCondition(recall=lognormalProbabilities.ipsur, message=gettextRcmdr("The standard deviation was not specified."))
              return()
        }
        doItAndPrint(paste("plnorm(c(", probabilities, "), meanlog=", logmu,
            ", sdlog=", logsigma, ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
...
lognormalDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        fun <- tclvalue(functionVar)
        if ( mulog =="" ){
              errorCondition(recall=lognormalDistributionPlot.ipsur, message=gettextRcmdr("The mean was not specified."))
              return()
        }
        if ( sigmalog ==""  ){
    ...
    tkgrid(tklabel(top, text=gettextRcmdr("meanlog (mean of dist'n on log scale)")), mulogEntry, sticky="e")
...
logisticQuantiles.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        quantiles <- gsub(" ", ",", tclvalue(quantilesVar))
        if ("" == quantiles) {
              errorCondition(recall=logisticQuantiles.ipsur, message=gettextRcmdr("No probabilities specified."))
              return()
        }
        location <- tclvalue(locationVar)
        scale1 <- tclvalue(scale1Var)
        tail <- tclvalue(tailVar)
        if ( location =="" ){
              errorCondition(recall=logisticQuantiles.ipsur, message=gettextRcmdr("The location parameter must be a real number."))
              return()
        }
        if ( scale1 ==""  ){
              errorCondition(recall=logisticQuantiles.ipsur, message=gettextRcmdr("The scale parameter was not specified."))
              return()
        }
...
logisticProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        probabilities <- gsub(" ", ",", tclvalue(probabilitiesVar))
        if ("" == probabilities) {
              errorCondition(recall=logisticProbabilities.ipsur, message=gettextRcmdr("No values specified."))
              return()
        }
        location <- tclvalue(locationVar)
        scale1 <- tclvalue(scale1Var)
        tail <- tclvalue(tailVar)
        if ( location =="" ){
              errorCondition(recall=logisticProbabilities.ipsur, message=gettextRcmdr("The location parameter was not specified."))
              return()
        }
        if ( scale1 ==""  ){
              errorCondition(recall=logisticProbabilities.ipsur, message=gettextRcmdr("The scale parameter was not specified."))
              return()
        }
...
logisticDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        fun <- tclvalue(functionVar)
        if ( location =="" ){
              errorCondition(recall=logisticDistributionPlot.ipsur, message=gettextRcmdr("The location parameter was not specified."))
              return()
        }
        if ( scale1 ==""  ){
...
normalQuantiles.ipsur  <- function(){
...
normalProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        tail <- tclvalue(tailVar)
        if ("" == probabilities) {
              errorCondition(recall=normalProbabilities.ipsur, message=gettextRcmdr("No values specified."))
              return()
        }
        if ( mu =="" ){
              errorCondition(recall=normalProbabilities.ipsur, message=gettextRcmdr("The mean was not specified."))
              return()
        }
        if ( sigma =="" ){
              errorCondition(recall=normalProbabilities.ipsur, message=gettextRcmdr("The standard deviation was not specified."))
              return()
        }
...
normalDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        fun <- tclvalue(functionVar)
        if ( mu =="" ){
              errorCondition(recall=normalDistributionPlot.ipsur, message=gettextRcmdr("The mean was not specified."))
              return()
        }
        if ( sigma =="" ){
...
tQuantiles.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        tail <- tclvalue(tailVar)
        doItAndPrint(paste("qt(c(", quantiles, "), df=", df, ", ncp=", ncp,
            ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
...
tProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        tail <- tclvalue(tailVar)
        doItAndPrint(paste("pt(c(", probabilities, "), df=", df, ", ncp=", ncp,
            ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
...
tDistributionPlot.ipsur  <- function(){
...
uniformQuantiles.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        #        if ( min1 > max1 ){
        #        errorCondition(recall=uniformQuantiles.ipsur,
        #message=gettextRcmdr("The upper limit(max) cannot be less than the lower limit(min)."))
        #                        return()
        #                    }
        doItAndPrint(paste("qunif(c(", quantiles, "), min=", min1,
            ", max=", max1, ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
    OKCancelHelp(helpSubject="qunif")
    tkgrid(tklabel(top, text=gettextRcmdr("Probabilities")), quantilesEntry, sticky="e")
    tkgrid(tklabel(top, text=gettextRcmdr("min (lower limit of the distribution)")), min1Entry, sticky="e")
...
uniformProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        probabilities <- gsub(" ", ",", tclvalue(probabilitiesVar))
        if ("" == probabilities) {
              errorCondition(recall=uniformProbabilities.ipsur, message=gettextRcmdr("No values specified."))
              return()
        }
        min1 <- tclvalue(min1Var)
        max1 <- tclvalue(max1Var)
        tail <- tclvalue(tailVar)
        if ( min1 =="" ){
              errorCondition(recall=uniformProbabilities.ipsur, message=gettextRcmdr("The lower limit(min) was not specified."))
              return()
        }
        if (  max1 =="" ){
              errorCondition(recall=uniformProbabilities.ipsur, message=gettextRcmdr("The upper limit(max) was not specified."))
              return()
        }
        #     if ( min1 > max1 ){
        ...
        #                        return()
        #                    }
        doItAndPrint(paste("punif(c(", probabilities, "), min=", min1,
            ", max=", max1, ", lower.tail=", tail == "lower",")", sep=""))
        tkfocus(CommanderWindow())
        }
...
unifDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        fun <- tclvalue(functionVar)
        if ( min1 =="" ){
              errorCondition(recall=unifDistributionPlot.ipsur, message=gettextRcmdr("The lower limit(min) was not specified."))
              return()
        }
        if ( max1 =="" ){
...
weibullQuantiles.ipsur  <- function(){
...
weibullProbabilities.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        probabilities <- gsub(" ", ",", tclvalue(probabilitiesVar))
        if ("" == probabilities) {
            errorCondition(recall=weibullProbabilities.ipsur, message=gettextRcmdr("No values specified."))
            return()
            }
        shape <- tclvalue(shapeVar)
        scale1 <- tclvalue(scale1Var)
        tail <- tclvalue(tailVar)
        if ( shape =="" ){
              errorCondition(recall=weibullProbabilities.ipsur, message=gettextRcmdr("The shape parameter was not specified."))
              return()
        }
        if ( scale1 =="" ){
              errorCondition(recall=weibullProbabilities.ipsur, message=gettextRcmdr("The scale parameter was not specified."))
              return()
        }
...
weibullDistributionPlot.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        fun <- tclvalue(functionVar)
        if ( shape =="" ){
              errorCondition(recall=weibullDistributionPlot.ipsur, message=gettextRcmdr("The shape parameter was not specified."))
              return()
        }
        if ( scale1 =="" ){
...
birthdayProbabilities.ipsur  <- function(){
    ...
    probButton <- tkradiobutton(distFrame, variable=optionVar, value="probability")
    quantileButton <- tkradiobutton(distFrame, variable=optionVar, value="quantile")
        ...
        option <- tclvalue(optionVar)
              ...
              if (option =="probability"){
                   if ( is.na(n) ){
                    ...
                    doItAndPrint(paste("pbirthday.ipsur(n=", n,
                        ", classes=", classes, ", coincident=", coincident,")", sep=""))
                    tkfocus(CommanderWindow())
              }
    ...
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters"), fg="red"), columnspan=6, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("classes (distinct categories)")), classesEntry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("coincident (num people falling in same category) ")), coincidentEntry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr(""), fg="red"), columnspan=6, sticky="w")
    tkgrid(tklabel(distFrame, text=gettextRcmdr("Select Option"), fg="red"), columnspan=6, sticky="w")
    tkgrid.configure(classesEntry, sticky="w")
    tkgrid.configure(coincidentEntry, sticky="w")
    tkgrid(tklabel(probFrame, text=gettextRcmdr("n (num of people) = ")),probField, sticky="w")
    tkgrid(tklabel(distFrame, text=gettextRcmdr("Probabilities"), fg="blue"), probButton, probFrame, sticky="w")
    tkgrid(tklabel(quantileFrame, text=gettextRcmdr("prob (of coincidence) = ")), quantileField, sticky="w")
    tkgrid(tklabel(distFrame, text=gettextRcmdr("Quantiles"), fg="blue"), quantileButton, quantileFrame, sticky="w")
    tkgrid(distFrame, sticky="w")
    tkgrid(buttonsFrame, sticky="w")
    dialogSuffix(rows=6, columns=2, focus=classesEntry)
}
##############################################################
#  End Birthday problem
