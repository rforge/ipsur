...
# change .path.package(package="IPSUR")
...
Commander <- function(){
    ...
    etc <- setOption("etc", file.path(.path.package(package="IPSUR")[1], "etc"), global=FALSE)
    ...
    ###########################################################################
    setOption("check.packages", FALSE)
    ###########################################################################
    ...
    for (m in 1:nrow(Menus)){
        ...
        if (Menus[m, 1] == "menu") {
            ...
            if (Menus[m, 3] == "command"){
                ...
                tkadd(eval(parse(text=Menus[m, 2])),"cascade", label=gettextRcmdr(Menus[m, 4]), menu=eval(parse(text=Menus[m, 5])))
