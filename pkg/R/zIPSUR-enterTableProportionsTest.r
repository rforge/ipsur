...
enterTableSinglePropTest <- function(){
    require("abind")
    env <- environment()
    ...
    rowsValue <- tclVar("1")
    rowsSlider <- tkscale(rowColFrame, from=1, to=1, showvalue=FALSE, variable=rowsValue,
    ...
    onOK <- function(){
     ...
     # table setup
        ...
        if (length(counts) != nrows*ncols){
            errorCondition(recall=enterTableSinglePropTest, message=sprintf(gettextRcmdr("Number of valid entries (%d)\nnot equal to number of rows (%d) * number of columns (%d)."), length(counts), nrows, ncols))
            return()
            }

        if (length(unique(col.names)) != ncols){
            errorCondition(recall=enterTableSinglePropTest, message=gettextRcmdr("Column names are not unique."))
            return()
            }
    ...
    #######################################################################################
    # table setup
    #tkgrid(tklabel(rowColFrame, text=gettextRcmdr("Enter counts:"), fg="blue"), sticky="w")
    tkgrid(tklabel(rowColFrame, text=gettextRcmdr("Number of Rows (single sample):")), rowsSlider, rowsShow, sticky="w")
    tkgrid(rowColFrame, sticky="w")

    tkgrid(outerTableFrame, sticky="w")

    tkgrid(tklabel(top, text=gettextRcmdr("\nOptions:"), fg="blue"), sticky="w")

    tkgrid(tklabel(pFrame, text=gettextRcmdr("Null hypothesis: p0 = ")), pField, sticky="w")
...
enterTableMultiPropTest <- function(){
    require("abind")
    env <- environment()
    ...
    setUpTable <- function(...){
        ...
        for (i in 1:nrows){
            ...
            assign(varname, tclVar("") , envir=env)
            row.varname <- paste(".rowname.", i, sep="")
            assign(row.varname, tclVar(paste("Sample ", i, sep="")), envir=env)
    ...
    onOK <- function(){
     ...
     # table setup
        ...
        counts <- na.omit(counts)
        if (length(counts) != nrows*ncols){
            errorCondition(recall=enterTableMultiPropTest, message=sprintf(gettextRcmdr("Number of valid entries (%d)\nnot equal to number of rows (%d) * number of columns (%d)."), length(counts), nrows, ncols))
            return()
            }
        if (length(unique(row.names)) != nrows){
            errorCondition(recall=enterTableMultiPropTest, message=gettextRcmdr("Row names are not unique."))
            return()
            }
        if (length(unique(col.names)) != ncols){
            errorCondition(recall=enterTableMultiPropTest, message=gettextRcmdr("Column names are not unique."))
            return()
            }
    ...
    radioButtons(name="alternative", buttons=c("twosided", "less", "greater"), values=c("two.sided", "less", "greater"),
        labels=gettextRcmdr(c("Two-sided", "Difference < 0 (samples=2)", "Difference > 0 (samples=2)")), title=gettextRcmdr("Alternative Hypothesis"))
    #rightFrame <- tkframe(top)
    confidenceFrame <- tkframe(top)
    confidenceLevel <- tclVar("0.95")
    confidenceField <- tkentry(confidenceFrame, width="6", textvariable=confidenceLevel)
    radioButtons(name="test", buttons=c("normal", "corrected"),
        labels=gettextRcmdr(c("Normal approximation", "Normal approximation with\ncontinuity correction (samples=2)")), title=gettextRcmdr("Type of Test"))

    #######################################################################################
    # table setup
    #tkgrid(tklabel(rowColFrame, text=gettextRcmdr("Number of Columns (reponses):")), colsSlider, colsShow, sticky="w")
    tkgrid(tklabel(rowColFrame, text=gettextRcmdr("Number of Rows (samples):")), rowsSlider, rowsShow, sticky="w")
    tkgrid(rowColFrame, sticky="w")
    ...
    #################################################################################################
    tkgrid(tklabel(confidenceFrame, text=gettextRcmdr("Confidence Level (samples=2): ")), confidenceField, sticky="w")
    tkgrid(confidenceFrame, sticky="w")
    tkgrid(alternativeFrame, sticky="nw")
    tkgrid(testFrame, sticky="w")
    tkgrid(buttonsFrame, columnspan=2, sticky="w")
