...
# IPSUR-Probability menu dialogs for selecting samples and computing statistics

# modified 26 July 06 by J. Fox
# changed for IPSUR September 2006

normalDistributionSamples.ipsur <- function(){
    ...
    otherstat2Entry <- tkentry(otherstatFrame, width="12", textvariable=otherstat2)
    otherstat3Entry <- tkentry(otherstatFrame, width="12", textvariable=otherstat3)
    ...
    onOK <- function(){
        ...
        if ( sigma == "" ) {
            errorCondition(recall=normalDistributionSamples.ipsur, message=gettextRcmdr("Standard deviation not specified."))
            return()
            }
        if ( mu == "" ) {
            errorCondition(recall=normalDistributionSamples.ipsur, message=gettextRcmdr("Mean not specified."))
            return()
            }
        if ( n == "" ) {
            errorCondition(recall=normalDistributionSamples.ipsur,
        ...
        if (tclvalue(meanVariable) == "1") {
            command <- paste(dsnameValue, "$mean <- rowMeans(as.matrix(", dsnameValue,
                "[,1:", n, "]))", sep="")
            justDoIt(command)
            logger(command)
            }
        if (tclvalue(sumVariable) == "1") {
            command <- paste(dsnameValue, "$sum <- rowSums(as.matrix(", dsnameValue,
                "[,1:", n, "]))", sep="")
            justDoIt(command)
            logger(command)
            }
        if (tclvalue(sdVariable) == "1") {
            command <- paste(dsnameValue, "$sd <- apply(as.matrix(", dsnameValue,
                "[,1:", n, "]), 1, sd)", sep="")
            justDoIt(command)
            logger(command)
            }
        ...
        if (tclvalue(otherstatVariable) == "1") {
                    ...
                    if ( otherst != "" ){
                    command <- paste(dsnameValue, "$", otherst, " <- apply(as.matrix(", dsnameValue,
                        "[,1:", n, "]), 1, ", otherst, ")", sep="")
                    justDoIt(command)
                    logger(command)
                    }
                    ...
                    if ( otherst2 != "" ){
                    command <- paste(dsnameValue, "$", otherst2, " <- apply(as.matrix(", dsnameValue,
                        "[,1:", n, "]), 1, ", otherst2, ")", sep="")
                    justDoIt(command)
                    logger(command)
                    }
                    ...
                    command <- paste(dsnameValue, "$", otherst3, " <- apply(as.matrix(", dsnameValue,
                        "[,1:", n, "]), 1, ", otherst3, ")", sep="")
                    justDoIt(command)
                    logger(command)
                    }
        ...
        if ( tclvalue(discardVariable) == "1") {
              if (n == 1){
                     command <- paste( dsnameValue, '$obs <- NULL', sep="") 
                     justDoIt( command )
              } else { 
                     variables = paste("obs", 1:as.numeric(n), sep="")
                      for ( k in 1:as.numeric(n)){              
                            eval(parse(text=paste(dsnameValue, "$obs", k, "<- NULL", sep="")), envir=.GlobalEnv) 
                      }
              }
              logger(paste("The original observations were discarded.", sep=""))
        }
        ####################################################################################
    ...
    tkgrid(tklabel(top, text=gettextRcmdr("Enter name for data set:"), fg="blue"), entryDsname, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("")), columnspan=2, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("Dimensions:"), fg="blue"), columnspan=2, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("Number of samples (rows) ")), samplesEntry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("Sample size (columns) ")), nEntry, sticky="w")
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters:"), fg="blue"), columnspan=2, sticky="w")
    ...
    #########################################################################################
    tkgrid(tklabel(otherstatFrame, text=gettextRcmdr("Other sample statistic(s)      ") ), otherstatCheckBox,
        tklabel(otherstatFrame, text=gettextRcmdr(" Specify:"), fg="blue"), otherstatEntry, sticky="w")
    tkgrid(tklabel(otherstatFrame, text=gettextRcmdr("")), tklabel(otherstatFrame, text=gettextRcmdr("")),
        tklabel(otherstatFrame, text=gettextRcmdr("")), otherstat2Entry, sticky="w")
    tkgrid(tklabel(otherstatFrame, text=gettextRcmdr("")), tklabel(otherstatFrame, text=gettextRcmdr("")),
        tklabel(otherstatFrame, text=gettextRcmdr("")), otherstat3Entry, sticky="w")
...
disunifDistributionSamples.ipsur  <- function(){
    ...
    onOK <- function(){
        ...
        if ( from1 == "" ) {
            errorCondition(recall=disunifDistributionSamples.ipsur, message=gettextRcmdr("Parameter 'from' not specified."))
            return()
            }
        if ( to1 == "" ) {
            errorCondition(recall=disunifDistributionSamples.ipsur, message=gettextRcmdr("Parameter 'to' not specified."))
            return()
            }
        if ( by1 == "" ) {
            errorCondition(recall=disunifDistributionSamples.ipsur, message=gettextRcmdr("Parameter 'by' not specified."))
            return()
            }
        if ( n == "" ) {
            errorCondition(recall=disunifDistributionSamples.ipsur,
        ...
        command <- paste(dsnameValue, " <- as.data.frame(matrix(sample(support, size=", samples, "*", n, ", replace=TRUE), ncol=", n, "))", sep="")
        justDoIt(command)
        logger(command)
        ##########################################################################
        command <- if (samples == 1)
...
tDistributionSamples.ipsur <- function(){
...
#            }
        ...
        command <- paste(dsnameValue, " <- as.data.frame(matrix(rt(", samples, "*", n, ", df=", df, ", ncp=", ncp, "), ncol=", n, "))", sep="")
        justDoIt(command)
        logger(command)
        command <- if (samples == 1) 
            paste("rownames(", dsnameValue, ') <- "sample"', sep="")
    ...
    tkgrid(tklabel(top, text=gettextRcmdr("")), columnspan=4, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("Dimensions:"), fg="blue"), columnspan=4, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("Number of samples (rows) ")), samplesEntry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("Sample size (columns) ")), nEntry, sticky="w")
    tkgrid(tklabel(top, text=""))
...
chisquareDistributionSamples.ipsur <- function(){
...
#            }
        ...
        command <- paste(dsnameValue, " <- as.data.frame(matrix(rchisq(", samples, "*", n, ", df=", df, ", ncp=", ncp, "), ncol=", n, "))", sep="")
        justDoIt(command)
        logger(command)
        command <- if (samples == 1) 
            paste("rownames(", dsnameValue, ') <- "sample"', sep="")
...
FDistributionSamples.ipsur <- function(){
...
#            }
        ...
        command <- paste(dsnameValue, " <- as.data.frame(matrix(rf(", samples, "*", n, ", df1=", df1, ", df2=", df2, ", ncp=", ncp, "), ncol=", n, "))", sep="")
        justDoIt(command)
        logger(command)
        command <- if (samples == 1) 
            paste("rownames(", dsnameValue, ') <- "sample"', sep="")
    ...
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters:"), fg="blue"), columnspan=4, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("df1 (numerator degrees of freedom)")), df1Entry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("df2 (denominator degrees of freedom)")), df2Entry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("ncp (noncentrality parameter)")), ncpEntry, sticky="w")
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Add to Data Set:"), fg="blue"), sticky="w")
    tkgrid(checkBoxFrame, columnspan=2, sticky="w")
...
exponentialDistributionSamples.ipsur <- function(){
    ...
    tkgrid(tklabel(top, text=gettextRcmdr("Sample size (columns) ")), nEntry, sticky="w")
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Parameter:"), fg="blue"), columnspan=4, sticky="w")
...
uniformDistributionSamples.ipsur <- function(){
...
#            }
    ...
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters:"), fg="blue"), columnspan=4, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("min (lower limit of the distribution)")), minEntry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("max (upper limit of the distribution)")), maxEntry, sticky="w")
...
betaDistributionSamples.ipsur <- function(){
...
#            }
        ...
        command <- paste(dsnameValue, " <- as.data.frame(matrix(rbeta(", samples, "*", n, ", shape1=", shape1, ", shape2=", shape2, ", ncp=", ncp, "), ncol=", n, "))", sep="")
        justDoIt(command)
        logger(command)
        command <- if (samples == 1) 
            paste("rownames(", dsnameValue, ') <- "sample"', sep="")
...
CauchyDistributionSamples.ipsur <- function(){
...
logisticDistributionSamples.ipsur <- function(){
...
lognormalDistributionSamples.ipsur <- function(){
...
#            }
    ...
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters:"), fg="blue"), columnspan=4, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("meanlog (mean of dist'n on log scale)")), meanlogEntry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("sdlog (std dev of dist'n on log scale)")), sdlogEntry, sticky="w")
...
gammaDistributionSamples.ipsur <- function(){
...
#            }
        ...
        command <- paste(dsnameValue, " <- as.data.frame(matrix(rgamma(", samples, "*", n, ", shape=", shape, ", rate=", s, "), ncol=", n, "))", sep="")
        justDoIt(command)
        logger(command)
        command <- if (samples == 1) 
            paste("rownames(", dsnameValue, ') <- "sample"', sep="")
    ...
    tkgrid(tklabel(top, text=gettextRcmdr("rate (= 1/scale)")), sEntry, sticky="w")
...
WeibullDistributionSamples.ipsur <- function(){
...
## ---- Gumbel distribution
#
#gumbelSimulate.ipsur <- function(){
#    initializeDialog(title=gettextRcmdr("Simulate Gumbel variates"))
...
#        dsnameValue <- trim.blanks(tclvalue(dsname))
#        if (dsnameValue == "") {
#            errorCondition(recall=GumbelDistributionSamples.ipsur,
#                message=gettextRcmdr("You must enter the name of a data set."))
#            return()
#            }
#        if (!is.valid.name(dsnameValue)) {
...
binomialDistributionSamples.ipsur <- function(){
...
#            }
    ...
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters:"), fg="blue"), columnspan=4, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("size (number of trials)")), trialsEntry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("prob (probability of success)")), probEntry, sticky="w")
...
PoissonDistributionSamples.ipsur <- function(){
...
#            }
    ...
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Parameter:"), fg="blue"), columnspan=4, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("lambda (mean)")), meanEntry, sticky="w")
...
geomDistributionSamples.ipsur <- function(){
...
hyperDistributionSamples.ipsur <- function(){
...
#                    }
    ...
    tkgrid(tklabel(top, text=gettextRcmdr("Dimensions:"), fg="blue"), columnspan=4, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("Number of samples (rows) ")), samplesEntry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("Sample size (columns) ")), sampleSizeEntry, sticky="w")
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters:"), fg="blue"), columnspan=4, sticky="w")
...
negbinomialDistributionSamples.ipsur <- function(){
...
#            }
        ...
        #if ( trials < 0){
#              errorCondition(recall=negnegbinomialDistributionSamples.ipsur,
...
#          }
    ...
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Parameters:"), fg="blue"), columnspan=4, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("size (target number of successes)")), trialsEntry, sticky="w")
    tkgrid(tklabel(top, text=gettextRcmdr("prob (probability of success)")), probEntry, sticky="w")
    tkgrid(tklabel(top, text=""))
    tkgrid(tklabel(top, text=gettextRcmdr("Add to Data Set:"), fg="blue"), sticky="w")
    tkgrid(checkBoxFrame, columnspan=2, sticky="w")
