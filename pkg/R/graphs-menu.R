...
indexPlot <- function(){
    ...
    onOK <- function(){
        ...
        command <- paste("plot(", .activeDataSet, "$", x, ', main="', title,'", type="', type, '")', sep="")
        ############################################################
...
Histogram <- function(){
    ...
    onOK <- function(){
        ...
        command <- paste("Hist(", ActiveDataSet(), "$", x, ', scale="',
            scale, '", breaks=', bins, ', main="',title,'", col="darkgray")', sep="")
        doItAndPrint(command)
        activateMenus()
        tkfocus(CommanderWindow())
        }
...
boxPlot <- function(){
    ...
    onOK <- function(){
        ...
        if (.groups == FALSE) {
            ...
            command <- (paste("boxplot(", var, ', ylab="', x,
                              '", main="', title, '", notch=', 
                              nflag, ', warwidth=', vwflag, ', 
                              horizontal=', hflag, ')', sep=""))
            #########################
            logger(command)
            justDoIt(command)
            if (identifyPoints) {
            ...
            command <- (paste("boxplot(", x, "~", .groups, ', ylab="', x,
                '", xlab="', .groups,'"', ', main="',title,'", notch=', nflag,
                ', varwidth=', vwflag, ', horizontal=', hflag, ', data=', .activeDataSet, ")", sep=""))
            #########################
            logger(command)
            justDoIt(command)
            if (identifyPoints) {
    ...
    ###################################################################
    optionsFrame <- tkframe(top)
    checkBoxes(frame="optionsFrame", boxes=c("horzOption", "notchOption", "varwidthOption"), 
        initialValues=c("1","0", "1"),
        labels=gettextRcmdr(c("Horizontal", "Notches", "Variable Box Width")))
    titleFrame <- tkframe(top)
    titleVariable <- tclVar(gettextRcmdr(""))
    titleField <- tkentry(titleFrame, width="40", textvariable=titleVariable)
    tkgrid(tklabel(titleFrame, text=gettextRcmdr("Title: "), fg="blue"), titleField, sticky="w")
...
scatterPlot <- function(){
    ...
    onOK <- function(){
        ...
        if (.groups == FALSE) {
            ...
            doItAndPrint(paste("scatterplot(", y, "~", x,
                ", reg.line=", line, ", smooth=", smooth, ", labels=", labels,
                ", boxplots=", box, ", span =", span/100, jitter, ', main="', title,
                '", data=', .activeDataSet, subset, ")", sep=""))
            #####################################################
            }
        else {
            ...
            doItAndPrint(paste("scatterplot(", y, "~", x," | ", .groups,
                ", reg.line=", line, ", smooth=", smooth, ", labels=", labels,
                ", boxplots=", box, ", span=", span/100, jitter,
                ", by.groups=", .linesByGroup, ', main="', title,
                '", data=', .activeDataSet, subset, ")", sep=""))
            #####################################################
            }
        activateMenus()
    ...
    onOK <- function(){
        ...
        if (.groups == FALSE) {
           ...
           command <- paste("scatterplot.matrix(~", paste(variables, collapse="+"),
                ", reg.line=", line, ", smooth=", smooth,
                ", span=", span/100, ", diagonal = '", diag,
                "', main='", title, "', data=", .activeDataSet, subset, ")", sep="")
           ######################################################
           logger(command)
           justDoIt(command)
            }
...
paretoChart <- function(){
    ...
    onOK <- function(){
        ...
        ############################################################
        command <- paste("pareto.chart(table(", ActiveDataSet(), "$", variable, '), main="', 
        title, '", ylab="Frequency"',colcomm, sep="")
        #logger(command)
        #justDoIt(command)
        doItAndPrint(command)
        activateMenus()
...
# this function modified by Martin Maechler
{
    ...
    onOK <- function(){
       ...
       if (0 == length(x)) {
        ...
        #                  ", labels=", labels, ")", sep="")
        ####################################################
        command <- paste("qq.plot", "(", .activeDataSet, "$", x, ", ", args, ', main="', title,
                          '", labels=', labels, ")", sep="")
        ####################################################
        doItAndPrint(command)
        activateMenus()
    ...
    titleVariable <- tclVar(gettextRcmdr(""))
    titleField <- tkentry(titleFrame, width="40", textvariable=titleVariable)
    tkgrid(tklabel(titleFrame, text=gettextRcmdr("Title: ")), titleField, sticky="w")
    tkgrid(titleFrame, sticky="w")
    ###################################################################
...
Xyplot <- function() {
    ...
    onOK <- function() {
        ...
        xyplot.command <- paste("xyplot(",
                                ...
                                ########################################
                                ', main="', title, '", data=', .activeDataSet, ')', sep="")
                                #################################
...
stripChart <- function(){
    ...
    slider1 <- tkscale(optionsFrame, from=1, to=25, showvalue=TRUE, variable=slider1Value,
        resolution=1, orient="horizontal")
    slider2Value <- tclVar("1")
    slider2 <- tkscale(optionsFrame, from=0.1, to=2.5, showvalue=TRUE, variable=slider2Value,
    ...
    onOK <- function(){
        ...
        jitter <- if (method == "jitter") paste(", jitter=", tclvalue(jitterVariable), sep="") else ""
        offset <- if (method == "stack") paste(", offset=", tclvalue(offsetVariable), sep="") else ""
        pch <- as.numeric(tclvalue(slider1Value))
        cex <- as.numeric(tclvalue(slider2Value))
        tkdestroy(top)
        if (.groups == FALSE) {
            doItAndPrint(paste("stripchart(", .activeDataSet,"$", x,
                ', method="', method, '"',jitter, offset,
                ', main="', title, '", pch=', pch, ', cex=', cex, ")", sep=""))
            #####################################################
            } else {
            doItAndPrint(paste("stripchart(", .activeDataSet,"$", x,"~",
                .activeDataSet, "$", .groups,
                ', method="', method, '"',jitter, offset,
                ', main="', title, '", pch=', pch, ', cex=', cex, ")", sep=""))
            #####################################################
            }
    ...
    overplotButton <- tkradiobutton(optionsFrame, variable=methodVariable, value="overplot")
    jitterButton <- tkradiobutton(optionsFrame, variable=methodVariable, value="jitter")
    stackButton <- tkradiobutton(optionsFrame, variable=methodVariable, value="stack")
    ...
    ###################################################################
    tkgrid(getFrame(xBox), sticky="nw")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Plot method:"), fg="blue"), sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Overplot")), overplotButton, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Jitter")), jitterButton,
        tklabel(optionsFrame, text=gettextRcmdr("   amount:")), jitterEntry, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Stack")), stackButton,
        tklabel(optionsFrame, text=gettextRcmdr("   offset:")), offsetEntry, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Plot Character:"), fg="blue"),tklabel(optionsFrame, text=gettextRcmdr("")), slider1, sticky="w")
    tkgrid(tklabel(optionsFrame, text=gettextRcmdr("Character Size:"), fg="blue"),tklabel(optionsFrame, text=gettextRcmdr("")), slider2, sticky="w")
    tkgrid(optionsFrame, sticky="w")
    tkgrid(groupsFrame, sticky="w")
    tkgrid(buttonsFrame, columnspan=2, sticky="w")
    dialogSuffix(rows=6, columns=2)
